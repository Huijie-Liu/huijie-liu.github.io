...
>
> ![img](./assets/(null)-20240401103437163-1938877.(null))

# Q1: how to learn image representations? 

## Overview

![img](./assets/(null)-20240401103437163.(null))

![img](./assets/(null)-20240401103437191.(null))

## 改进 CLIP

### 数据层面：Data scaling up

![img](./assets/(null)-20240401103437202.(null))
...
- image side
  ...
  - ![img](./assets/(null)-20240401103437302.(null))
- language side
  ...
  - ![img](./assets/(null)-20240401103437199.(null))
...
![img](./assets/(null)-20240401103437291.(null))
...
![img](./assets/(null)-20240401103437370.(null))
...
![img](./assets/(null)-20240401103437379.(null))
...
- adding a generative branch
  - Coca：通过在预训练阶段同时利用图像-文本对和图像-标签对来学习图像和文本的表征。并增加了一个生成分支，以提升模型性能并赋予其新的能力，如图像描述和视觉问答。
    ...
    - ![img](./assets/(null)-20240401103437347.(null))
...
- SimCLR：对于给定的图像，应用两种不同的数据增强方法，生成两个变体。这两个变体被视为正样本对，即它们代表同一图像的不同视角，并进行对比训练。
  - ![img](./assets/(null)-20240401103437452.(null))
- DeCLIP：
  - 自监督学习：
  ...
  - ![img](./assets/(null)-20240401103437543.(null))
- SLIP：SLIP 的核心思想是将 SimCLR和 CLIP相结合进行模型训练。
  - ![img](./assets/(null)-20240401103437548.(null))
- xCLIP： xCLIP = CLIP + nCLIP，是对 CLIP 的一个扩展，它利用图像自监督学习的技术来实现非对比学习，通过正则化和高维投影来确保学到的表示具有足够的区分度和鲁棒性。 引入了**锐度**（sharpness）和**平滑性**（smoothness）正则化。锐度正则化鼓励模型学习到尖锐的、区分度高的特征，而平滑性正则化则确保模型不会对数据的微小变化过度敏感。
  - ![img](./assets/(null)-20240401103437625.(null))
...
- BEiT：将自然语言处理中的预训练技术引入到图像领域，并通过视觉标记的概念建立起图像和文本之间的联系。
  ...
  - ![img](./assets/(null)-20240401103437865.(null))
- MAE：使用像素值作为目标来训练模型。
  ...
  - ![img](./assets/(null)-20240401103437772.(null))
...
- MVP：使用 CLIP 的图像特征作为 MIM 训练的目标，可以捕获在MIM训练中缺失的语义，这种做法能够结合两种方法的优势，提升模型在捕捉图像语义信息方面的能力。
  - ![img](./assets/(null)-20240401103437778.(null))
- EVA：推广了这一方法
  - ![img](./assets/(null)-20240401103437893.(null))
- Masked Autoencoding Does Not Help Natural Language Supervision at Scale：MAE对大规模的自然语言监督没有帮助。
  - ![img](./assets/(null)-20240401103437820.(null))
- BEiT-3：结合了 BERT和 BEiT的思想，进行多模态数据的遮蔽建模。
  ...
  - ![img](./assets/(null)-20240401103438009.(null))
...
![img](./assets/(null)-20240401103437873.(null))
...
![img](./assets/(null)-20240401103438016.(null))

### 数据

![img](./assets/(null)-20240401103437915.(null))

### 如何解决

![img](./assets/(null)-20240401103437965.(null))

## Bridge Vision with Language

学习原始视觉信号与丰富语义之间的映射，并可以对各种开放词汇视觉识别任务提供动力

![img](./assets/(null)-20240401103438207.(null))

![img](./assets/(null)-20240401103438180.(null))

- GroupViT：通过从头开始学习图像-文本对来学习对语义相似的区域进行分组
  - 使用分组块进行自下而上的分组
  - 自上而下的图像-文本监督以实现视觉语义对齐
  - ![img](./assets/(null)-20240401103438234.(null))
- MaskCLIP：从CLIP中提取自由的密集标签
  - 将注意力池更改为新的适应策略
  - 使用CLIP作为教师模型的伪标签掩码
  - ![img](./assets/(null)-20240401103438117.(null))
- OpenSeg：弱监督学习，通过强制文本特征和掩码池特征之间的细粒度对齐。
  - 从图像-文本对和局部描述中学习。
  - 使用预训练的掩码建议网络。
  - ![img](./assets/(null)-20240401103438497.(null))
- MaskCLIP (UCSD)：使用CLIP作为初始化的COCO全视分割的监督训练
  - 掩码建议网络训练
  - CLIP模型自适应
  - ![img](./assets/(null)-20240401103438086.(null))
- 总结
  - CLIP作为基础对开放词汇训练有很大帮助
  - 将弱注释与黄金注释结合起来以获得更好的性能
  - ![img](./assets/(null)-20240401103438233.(null))

## Unified multi-task modeling

希望开发一个统一的视觉模型，该模型可以在许多视觉任务中表现良好。

![img](./assets/(null)-20240401103438335.(null))

两种统一化

![img](./assets/(null)-20240401103438177.(null))

- Outputs Unification
  - 概述
    - 以一致的格式表示不同输出（一个输出里包含多个任务的结果）
    - ![img](./assets/(null)-20240401103438596.(null))
  -  UniTab and Pix2Seqv2
    - **统一词表：** 将文本和坐标都进行标记化（tokenization），并放入同一个词表中。
    - **任务前缀：** 需要一个任务前缀来确定模型正在处理的任务
    - ![img](./assets/(null)-20240401103438974.(null))
  - Unified-IO：加入了VQVAE
    ...
    - ![img](./assets/(null)-20240401103438294.(null))
  - 总结
...
- Functionality Unification
  - 概述
    ...
    - ![img](./assets/(null)-20240401103438373.(null))
  - UniPerceiver-v2
    - ![img](./assets/(null)-20240401103438644.(null))
  - X-Decoder：允许图像级和像素级的监督信号相互作用。
    - ![img](./assets/(null)-20240401103438720.(null))
...
- SAM
  - ![img](./assets/(null)-20240401103438692.(null))
- SEEM: Segment Everything Everywhere all at Once
  - ![img](./assets/(null)-20240401103439015.(null))
...
- 4大主题
  - ![img](./assets/(null)-20240401103439153.(null))
- 研究总结
  - ![img](./assets/(null)-20240401103438746.(null))
- T2I模型总览
  - ![img](./assets/(null)-20240401103439020.(null))
- DF原理
  - ![img](./assets/(null)-20240401103438973.(null))
...
1. **区域绑定的文本描述**：将传统T2I模型中的图像级文本描述扩展到区域绑定的文本描述，使得开放式文本描述能够精确地操作特定的空间区域。
   ...
   2. ![img](./assets/(null)-20240401103439045.(null))
...
2. **密集空间条件**：这一类研究从边界框扩展到以2D数组形式表示的密集空间条件，如分割掩码、边缘图、深度图和关键点等。
   ...
   2. ![img](./assets/(null)-20240401103439419.(null))
...
![img](./assets/(null)-20240401103439401.(null))
...
1. 推理时操作（Inference-time manipulation）:
   ...
   2. StructureDiffusion：使用解析树来提取名词短语和文本提示的语言结构，然后强制模型“关注”所有提取的名词短语。这是通过修改交叉注意力机制来实现的，其中 O = M · V，M 是 softmax 交叉注意力图，V 是句子特征。$$O=\frac1{k+1}\Sigma_{i=0}^k(M\cdot V_i)$$，其中 $$V_0$$ 是句子特征 ，$$V_i$$ 是解析树中的短语特征。这种方法确保视觉流在所有识别的名词短语上保持平衡的注意力，从而促进更准确的图像生成。
      ...
      - ![img](./assets/(null)-20240401103439330.(null))
   ...
   4. ![img](./assets/(null)-20240401103439425.(null))
...
2. 对齐调优（Alignment tuning）:
   ...
   2. DDPO：加入强化学习
      ...
      - ![img](./assets/(null)-20240401103439944.(null))
...
1. 单一概念定制: 从单一概念定制开始，该过程涉及测试时微调，将视觉概念的多张图片编码成新的Token嵌入。
   ...
   2. Dreambooth：仅调整输入图像可能会导致对特定概念的 T2I 模型过度拟合的风险，为了解决这个问题，本文提出了特定于类的先验保留损失。这种方法的核心是使用预训练的 T2I 模型来生成与目标定制概念相同的类图像。然后，该模型在输入图像（使用 [V] 令牌）和模型生成的图像（没有 [V] 令牌）上联合微调。确保了模型能够在独特的“[V] 狗”和它最初训练的其他一般狗之间进行区分，同时保持其整体的T2I能力。
      ...
      - ![img](./assets/(null)-20240401103439445.(null))

      - ![img](./assets/(null)-20240401103439411.(null))
...
2. 多概念定制: 允许扩展文本到图像模型的Token词汇表，以包括多个概念Tokens，这使得多个概念能够在生成过程中相互作用以及与剩余的视觉场景交互。
   ...
   1. Custom Diffusion
      - ![img](./assets/(null)-20240401103439746.(null))
...
3. 个性化测试微调的简化: 由于测试时微调要求用户为每个新概念定制T2I模型，这可能会变得相当复杂。为简化使用流程，一些研究探索了无需测试时微调的定制方法，采用统一的微调阶段来扩展T2I模型，使其接受图像条件输入。这些模型将视觉概念的图像作为额外的输入条件，并根据文本描述生成包含视觉概念的图像。
   ...
   1. SuTI：训练单个模型来模拟微调的主题特定专家，并生成以文本和主题输入图像为条件的图像。
      - ![img](./assets/(null)-20240401103440055.(null))
...
- **连接两个模态的可训练模块**：为了融合视觉与语言信息，模型需要有专门的可训练模块。常见的有：
  ...
  - ![img](./assets/(null)-20240401103439631.(null))
...
- LLaVA: Large Language-and-Vision Assistant
  - ![img](./assets/(null)-20240401103439681.(null))

## 总结

![img](./assets/(null)-20240401103439659.(null))
...
![img](./assets/(null)-20240401103439811.(null))

![img](./assets/(null)-20240401103439840.(null))

## New Paradigm

![img](./assets/(null)-20240401103439845.(null))

-  MM-ReAct: Prompting ChatGPT for Multimodal Reasoning and Action 
  - ![img](./assets/(null)-20240401103439972.(null))

  - ![img](./assets/(null)-20240401103440030.(null))